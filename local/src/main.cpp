...
float Hxy(std::vector<datatype> &y_true, std::vector<datatype> &y_pred)
{
    ...
    **/
    ...
    //      #endif
    //         // pxy[0].merge(pxy[i]);
    //         pxy[0] = std::accumulate(pxy[0].begin(), pxy[0].end(), pxy[i],
    //                                  [](std::map<std::pair<datatype, datatype>, float> &m, const std::pair<std::pair<datatype, datatype>, float> &p)
    //                                  {
    //                                      return (m[p.first] += p.second, m);
    //                                  });
    //         px[0] = std::accumulate(px[0].begin(), px[0].end(), px[i],
    //                                  [](std::map<datatype, float> &m, const std::pair<datatype, float> &p)
    //                                  {
    //                                      return (m[p.first] += p.second, m);
    //                                  });
    //         py[0] = std::accumulate(py[0].begin(), py[0].end(), py[i],
    //                                 [](std::map<datatype, float> &m, const std::pair<datatype, float> &p)
    //                                 {
    //                                     return (m[p.first] += p.second, m);
    //                                 });
    ...
    {
        auto pxx_iterator = std::next(pxy.begin(),i);
        datatype x = pxx_iterator->first.first;
        datatype y = pxx_iterator->first.second;
        float h = static_cast<float>(pxy[{x, y}]) * log(pxy[{x, y}] / (px[x] * py[y])); // cambio base log in ln /log(exp(1))
    #pragma omp critical
        {
            H+= h;
            #ifdef DEBUG
